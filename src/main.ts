 ...
 * @LastEditTime: 2021-11-14 15:57:45
 * @LastEditors: LSC
...
axios.interceptors.request.use((config) => {
  ...
  // 对象展开符代表前面的参数都需要
  // *get 请求，添加到 url 中
  config.params = { ...config.params, icode: 'C5806EDC427B0645' }
  // 其他请求，添加到 body 中
  // *如果是上传文件，添加到 FormData 中
  if (config.data instanceof FormData) {
    config.data.append('icode', 'C5806EDC427B0645')
  } else {
    // *普通的 body 对象，添加到 data 中 便捷的添加key-value
    config.data = { ...config.data, icode: 'C5806EDC427B0645' }
  }
  return config // 记得返回
})
